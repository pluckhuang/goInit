...
func TestUserHandler_LoginSMS(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			mock: func(ctrl *gomock.Controller) (service.UserService, service.CodeService) {
				usersvc := svcmocks.NewMockUserService(ctrl)
				codesvc := svcmocks.NewMockCodeService(ctrl)
				usersvc.EXPECT().FindOrCreate(gomock.Any(), phone).Return(domain.User{Id: 1}, nil)
				codesvc.EXPECT().Verify(gomock.Any(), bizLogin, phone, code).Return(true, nil)
				return usersvc, codesvc
			},
			reqBuilder: func(t *testing.T) *http.Request {
		...
		},
		{
			...
			mock: func(ctrl *gomock.Controller) (service.UserService, service.CodeService) {
				usersvc := svcmocks.NewMockUserService(ctrl)
				codesvc := svcmocks.NewMockCodeService(ctrl)
				codesvc.EXPECT().Verify(gomock.Any(), bizLogin, phone, code).Return(false, errors.New("模拟的错误"))
				return usersvc, codesvc
			},
			reqBuilder: func(t *testing.T) *http.Request {
				params := map[string]string{
		...
		},
		{
			...
			mock: func(ctrl *gomock.Controller) (service.UserService, service.CodeService) {
				usersvc := svcmocks.NewMockUserService(ctrl)
				codesvc := svcmocks.NewMockCodeService(ctrl)
				codesvc.EXPECT().Verify(gomock.Any(), bizLogin, phone, code).Return(false, nil)
				return usersvc, codesvc
			},
			reqBuilder: func(t *testing.T) *http.Request {
				params := map[string]string{
		...
		},
		{
			...
			mock: func(ctrl *gomock.Controller) (service.UserService, service.CodeService) {
				usersvc := svcmocks.NewMockUserService(ctrl)
				codesvc := svcmocks.NewMockCodeService(ctrl)
				usersvc.EXPECT().FindOrCreate(gomock.Any(), phone).Return(domain.User{}, errors.New("模拟的错误"))
				codesvc.EXPECT().Verify(gomock.Any(), bizLogin, phone, code).Return(true, nil)
				return usersvc, codesvc
			},
			reqBuilder: func(t *testing.T) *http.Request {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, tc.wantCode, recorder.Code)
			x := map[string]string{}
			json.Unmarshal(recorder.Body.Bytes(), &x)
			assert.Equal(t, tc.wantBody, x["msg"])
		})
	}
}
